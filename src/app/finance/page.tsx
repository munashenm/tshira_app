...
export default async function FinancePage() {
  // Show ALL active cases so admin can invoice any of them
  ...
  const stats = {
    ...
    totalValue: cases.filter(c => c.actualCost).reduce((acc, curr) => acc + (curr.actualCost || 0), 0)
  };
  ...
  return (
    <div className="p-8 space-y-8 bg-zinc-50 dark:bg-zinc-950 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <FinanceStatCard title="Ready to Invoice" value={stats.ready} icon={<Clock className="w-5 h-5" />} color="orange" />
        <FinanceStatCard title="Invoiced (Pending)" value={stats.invoiced} icon={<FileText className="w-5 h-5" />} color="blue" />
        <FinanceStatCard title="Total Paid" value={stats.paid} icon={<CheckCircle2 className="w-5 h-5" />} color="green" />
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-zinc-100 dark:border-zinc-800">
              ...
              <th className="px-8 py-5 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Workflow Status</th>
              <th className="px-8 py-5 text-[10px] font-black text-zinc-400 uppercase tracking-widest text-right">Invoice Actions</th>
            </tr>
          </thead>
              ...
              <tr key={c.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors group">
                <td className="px-8 py-6">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-zinc-300 font-mono mt-0.5">{c.nydaReference || c.id.slice(0,12)}</span>
                  </div>
                </td>
                <td className="px-8 py-6">
                  {c.invoiceNumber ? (
                    <Link href={`/finance/invoice/${c.id}`} target="_blank" className="text-xs font-mono font-bold text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded hover:underline">
                      {c.invoiceNumber}
                    </Link>
                  ) : (
                    <span className="text-xs text-zinc-300 italic">Not invoiced</span>
                  )}
                </td>
                <td className="px-8 py-6">
                  <span className="text-sm font-bold text-zinc-900 dark:text-zinc-50">
                    {c.actualCost ? `R ${c.actualCost.toLocaleString(undefined, {minimumFractionDigits: 2})}` : <span className="text-zinc-300 text-xs italic">Set on invoice</span>}
                  </span>
                </td>
                <td className="px-8 py-6">
            ...
            {cases.length === 0 && (
              <tr>
                <td colSpan={5} className="px-8 py-20 text-center">
                  ...
                  <p className="text-zinc-500 font-medium">No active work items found.</p>
                  <p className="text-xs text-zinc-400 mt-1">Create a new NYDA Case from the Dashboard to get started.</p>
                </td>
              </tr>
            )}
          </tbody>
...
function FinanceStatCard({ title, value, icon, color }: { title: string, value: number, icon: React.ReactNode, color: string }) {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${styles[color]}`}>{icon}</div>
...
function StatusBadge({ status }: { status: CaseStatus }) {
  const styles: Record<string, string> = {
    CLIENT_APPROVED: "bg-blue-50 text-blue-700",
    READY_FOR_INVOICING: "bg-orange-50 text-orange-700",
    INVOICED: "bg-purple-50 text-purple-700",
    PAID: "bg-emerald-50 text-emerald-700",
    RECEIVED_FROM_NYDA: "bg-zinc-100 text-zinc-500",
    ASSIGNED_TO_PROVINCE: "bg-sky-50 text-sky-600",
    INTERNALLY_REVIEWED: "bg-indigo-50 text-indigo-600",
    SENT_TO_NYDA: "bg-violet-50 text-violet-600",
  };
  return (
    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase ${styles[status] || 'bg-zinc-100 text-zinc-500'}`}>
      {status.replace(/_/g, ' ')}
    </span>
  );
}
